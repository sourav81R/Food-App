...
function CartItemCard({ data }) {
    ...
    return (
        <div className='w-[320px] shrink-0 overflow-hidden rounded-[28px] border border-orange-100 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:w-[360px]'>
            <div className='flex flex-col gap-4 bg-[linear-gradient(135deg,#fffaf6,#ffffff)] p-4'>
                <div className='flex min-w-0 items-center gap-4'>
                    <div className='overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm'>
                        <img src={data.image} alt={data.name} className='h-24 w-24 object-cover sm:h-28 sm:w-28' />
                    </div>
                    <div className='min-w-0'>
                        <h3 className='text-lg font-bold text-slate-900 sm:text-xl'>{data.name}</h3>
                        <p className='mt-1 text-sm text-slate-500'>Rs {data.price} x {data.quantity}</p>
                        <p className='mt-3 text-2xl font-bold text-slate-900'>Rs {lineTotal}</p>
                    </div>
                </div>
                ...
                <div className='flex flex-wrap items-center justify-between gap-3'>
                    ...
                    <button
                        className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 text-red-500 shadow-sm transition hover:bg-red-100"
